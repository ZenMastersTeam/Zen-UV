...
3. Мы создали швы в зависимости от остроты рёбер (edges), но еще не разделили модель на острова. Для разделения используем оператор [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap) работающий в зависимости от того, что выделено. На данном этапе убедитесь, что ничего не выделено. В режиме когда ничего не выделено [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap) просто разделяет модель на UV острова по существующим seams. Если что-то выделено, результат будет другим.
    - Убедитесь, что ничего не выделено.
    - Нажмите [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap) в панели [Unwrap](../../unwrap.md).

    ![](../../img/tutorial/emergency_light/initial_unwrap.gif)

Другие режимы работы [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap) мы рассмотрим в ходе этого туториала.
...
1. Начнём с цилиндрической детали. 
    ...
    - Выделите торец и нажмите кнопку [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap) из панели [Unwrap](../../unwrap.md)
...
2. Используем режим выделения **Edges** для развертывания следующего острова.
    ...
    - Переключите Blender в режим выделения **Edges**. В этом режиме [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap) создаст seams для выделенных рёбер и выполнит развёртку.
    - Выделите одно ребро на длинном цилиндре и нажмите [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap).
    ...
    Как видите [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap) точно придерживается ваших указаний и делает развёртку островов несмотря на то что мы изменяем тип выделения. Для этого даже не нужно открывать его настройки. В результате мы получили корректно развернутый самый большой остров этой детали..

3. Развернем второй торец детали. 
    - Переключите режим выделения Blender в **Faces**.
    - Выделите торец.
    - Нажмите [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap).

    ![](../../img/tutorial/emergency_light/cylindrical_second_cap.gif)

4. При использовании Mark by Angle были созданы нежелательные seams, что разделило модель на большее количество островов, чем необходимо. Чтобы исправить это, снова воспользуемся оператором [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap).
    - Выделите все полигоны самого большого цилиндра и нажмите [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap).
...
3. Созданный остров может выглядеть лучше если выровнять его в ровную решетку. Используйте оператор [Quadrify](../../transform_sys/independent_ops.md#quadrify-islands). Обратите внимание. Если у вас активирована опция **Use Selected Edges**, то лучше перейти в режим выделения **Faces**. Ведь в режиме выделения edges оператор Quadrify разорвёт остров по выделенным edges. Также обратите внимание на то, что Zen UV все время пытается вам помочь и для того, чтобы указать оператору с каким островом вы хотите работать, не обязательно выделять весь остров. Достаточно выделить один или несколько полигонов.
    - Выделите один или несколько полигонов в острове внутренней части текущей детали и нажмите [Quadrify](../../transform_sys/independent_ops.md#quadrify-islands).
...
2. На всех мелких деталях, кроме цилиндрических, оператор [Mark by Ange](../../unwrap.md#mark-by-angle) сработал корректно. Давайте исправим это.
    - Выделите по одному ребру на каждом из цилиндров и нажмите [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap).
...
5. На этом этапе мы получили корректное разделение на острова, но некоторые из них имеют растяжения, а другие — не оптимальную форму. Необходимо выделить и выровнять все острова, состоящие из квадов.
    ...
    - Используйте оператор [Quadrify](../../transform_sys/independent_ops.md#quadrify-islands) чтобы выровнять все выделенные острова в ровную решетку.
...
2. Видно что сейчас каждый провод разделен на несколько островов. Сделаем так, чтобы каждый провод был представлен ​​одним островом. 
    - Выделите все полигоны и нажмите [Zen Unwrap](../../unwrap/zen_unwrap.md#zen-unwrap).
...
3. Мы получили разделение, при котором каждый провод представлен отдельным островом, но развёртка выполнена некорректно. Давайте развернем провода самым быстрым способом.
    ...
    - Используйте оператор [Quadrify](../../transform_sys/independent_ops.md#quadrify-islands) чтобы выровнять каждый остров в ровную решетку.
...
4. Теперь у нас есть развёрнутый остров. Присутствие в нем треугольных faces не позволяет использовать оператор Quadrify для максимального разглаживания. Рассмотрим на примере этого острова работу оператора [Reshape Island](../../transform_sys/independent_ops.md#reshape-island). Суть работы [Reshape Island](../../transform_sys/independent_ops.md#reshape-island) заключается в том, что он разравнивает выделенные лупы рёбер в ровную линию, а остальные вершины расслабляет.
    - Полностью выделите ребра, проходящие вдоль центра острова.
    - Нажмите [Reshape Island](../../transform_sys/independent_ops.md#reshape-island) для выравнивания острова.
...
!!! Warning
    Перед тем как продолжить, убедитесь, что установлена **Zen UV Core Library**.  
    Без этой библиотеки результат стакования будет упрощённым.  
    Инструкцию по установке вы найдёте по ссылке:  
    [Установка Zen UV Core Library](https://zenmastersteam.github.io/Zen-UV/latest/installation/#zen-uv-core-library)
...
!!! Warning
    Для корректного запекания в UV Area не должно быть ни стаков, ни перекрытий. Должны остаться только [Singles](../../stack.md/#singles) и [Primaries](../../stack.md/#primaries). Все [Replicas](../../stack.md/#replicas) должны быть смещены на 1.0. Для этого удобно использовать оператор [Unstack](../../stack.md/#unstack). Однако этот процесс не является частью данного урока, и это предупреждение создано просто для напоминания.
